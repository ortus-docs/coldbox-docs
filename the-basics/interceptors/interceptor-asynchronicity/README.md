---
description: Asynchronous interception announcements using announce() arguments async, asyncAll, asyncAllJoin, asyncJoinTimeout, and asyncPriority.
---
...
| asyncAll         | boolean                    | false    | false         | Mutually exclusive with the async argument. If true, this will execute the interception point but multi-thread each of the classes that are listening to the interception point. So if 3 classes are listening, then 3 separate threads will be created for each class call. By default, the calling thread will wait for all 3 separate threads to finalize in order to continue with the execution. Extra arguments that can be used: _asyncAllJoin_, _asyncTimeout_, _asyncPriority_. |
| asyncAllJoin     | boolean                    | false    | true          | This flag is used only when combined with the asyncAll argument. If true (default), the calling thread will wait for all intercepted class calls to execute. If false, then the calling thread will not join the multi-threaded interception and continue immediate execution while the classes continue to execute in the background.                                                                                                                                               |
...
| asyncJoinTimeout | numeric                    | false    | 0             | This argument is only used when using the _asyncAll_ and _asyncAllJoin=true_ arguments. This argument is the number of milliseconds the calling thread should wait for all the threaded class listeners to execute. By default it waits until all threads finalize.                                                                                                                                                                                                                |
