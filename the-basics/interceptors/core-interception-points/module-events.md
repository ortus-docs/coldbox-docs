...
| preModuleRegistration  | {moduleRegistration, moduleName}       | This occurs right before a single module is registered (before its `ModuleConfig.bx` (or `.cfc` for CFML) is processed) |
| postModuleRegistration | {moduleConfig, moduleName}             | This occurs right after a single module has finished registering (its `ModuleConfig.bx` (or `.cfc` for CFML) has been processed and settings/mappings recorded) |
