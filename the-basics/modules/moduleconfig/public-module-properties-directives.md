...
| **parseParentSettings** | boolean | false    | true         | If true, ColdBox will merge any settings found in `moduleSettings[ this.modelNamespace ]` in the `config/ColdBox.bx` (or `.cfc` for CFML) file with the module settings, overriding them where the keys are the same.  Otherwise, settings in the module will override the parent configuration.                           |
...
| **cfmapping**           | string  | false    | empty        | The ColdFusion mapping that should be registered for you that points to the root of the module. **Deprecated in 8.1 — use `classMapping` instead.**                                                                                                                                                    |
| **classMapping**        | string  | false    | empty        | Engine-agnostic class mapping that points to the root of the module. Works on both CFML and BoxLang. Replaces the deprecated `cfmapping` property.                                                                                                                                                     |
